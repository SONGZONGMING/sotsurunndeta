...
real_subjects = unique(data{3});            % 1. 找出所有存在的编号(如 1, 2...31, 39...)
real_subjects(isnan(real_subjects)) = [];   % 2. 去除可能的无效值

for i = 1:length(real_subjects)
    sn = real_subjects(i);                  % 3. 取出当前要处理的编号
    
    ind = find(data{3} == sn);              % 4. 找数据
    
    if isempty(ind)
        continue;                           % 5. 双重保险：没找到就跳过
    end
...
for sn = 1:length(sub)
    % === 【新增修复】 如果这个被试是空的（比如32号），直接跳过 ===
    if isempty(sub(sn).subjectID)
        continue;
    end
    % ==========================================================
    % z-scored RT
