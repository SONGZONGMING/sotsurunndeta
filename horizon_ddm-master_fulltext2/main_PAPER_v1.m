...
clear
% === 修复路径：所有文件都在当前目录下 ===
% 2. 因为没有子文件夹，所有路径都指向当前这个文件夹
maindir = 'C:\D\SHUJU\horizon_ddm-master_fulltext2\';
fundir  = maindir;  % 代码就在这
datadir = maindir;  % 数据也在这

% 3. 设置 MATLAB 环境
addpath(genpath(fundir)); % 把当前文件夹及其包含的所有内容加入路径
cd(fundir);               % 确保 MATLAB 的"当前文件夹"切换到这里
...
%% load human data
sub = load_humanData_v1(datadir, 'allHorizonData_labo.csv', 'DDM_labo_fit.csv');
...
saveFigurePdf(gcf, './DDM_demographics')
...
saveFigurePdf(gcf, './pdf/DDMexample')
...
saveFigurePdf(gcf, './pdf/DDM_qualitative')
...
saveFigurePdf(gcf, './pdf/DDM_choices')
...
fprintf('正在检查数据完整性...\n');
if exist('sub', 'var')
    % 1. 标记空被试 或 有效试次过少(<10) 的被试
    bad_mask = false(size(sub));
    for i = 1:length(sub)
        if isempty(sub(i).subjectID) || isempty(sub(i).RT) || size(sub(i).a, 1) < 10
            bad_mask(i) = true;
        end
    end
    % 2. 剔除它们
    if any(bad_mask)
        fprintf('自动清洗：剔除 %d 个无效被试。\n', sum(bad_mask));
        sub(bad_mask) = [];
    end
end
% ==========================
...
saveFigurePdf(gcf, './pdf/DDM_params_absDR')
...
saveFigurePdf(gcf, './pdf/DDM_posteriorPredictiveABS')
...
saveFigurePdf(gcf, './pdf/DDM_parameterRecoveryABS_MLE')
...
saveFigurePdf(gcf, './pdf/ABS_ratios')
...
set(t, 'fontweight', 'normal', 'units', 'normalized')
set(t, 'position', [0.5000    1.08 0])
saveFigurePdf(gcf, './pdf/DDM_params2')
...
saveFigurePdf(gcf, './pdf/DDM_posteriorPredictiveMLEbest')
...
saveFigurePdf(gcf, './pdf/DDM_sigmaVsDDMpars2')
...
% ====== 比值输出 ======
fprintf('--- FIG8 ratios (per-subject ratios) ---\n');

fprintf('logistic sigma ratio (h1/h6) [1 3]: mean=%.3f, median=%.3f\n', mean(r13), median(r13));
fprintf('logistic sigma ratio (h1/h6) [2 2]: mean=%.3f, median=%.3f\n', mean(r22), median(r22));

fprintf('DDM cR^mu ratio (h6/h1): mean=%.3f, median=%.3f\n', mean(rA), median(rA));
fprintf('DDM c0^beta ratio (h6/h1): mean=%.3f, median=%.3f\n', mean(rZ), median(rZ));

%addABCs(ax, [-0.08 0.1], 28)
saveFigurePdf(gcf, './pdf/DDM_sigmaVsDDMpars')
% set(gca, 'xscale', 'log', 'yscale', 'log')
...
saveFigurePdf(gcf, './pdf/DDM_MLEvsMCMCparameters')
...
saveFigurePdf(gcf, './pdf/DDM_parameterRecoveryMLE')
...
% save ./pdf/DDM_fits_20200423_256
%%
...
saveFigurePdf(gcf, './pdf/DMM_modelComparison_MLE')
...
saveFigurePdf(gcf, './pdf/DDM_MLE_modelComparison')
...
dt = 0.001;
for sn = 1:length(fit_MLE)
    fak_MCMC(sn) = simulate_Mmodel_v1(fit_MLE(sn).XXfit, dt, fit_MLE(sn).dR, fit_MLE(sn).dI, fit_MLE(sn).gameLength);
    fak_MCMC(sn).sID = ones(size(fak_MCMC(sn).sID))*sub(sn).sNum;
end

%% write fake data out to CSV
fname = './DDM_labo_fit.csv';
